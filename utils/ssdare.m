function [K,V,rep,P] = ssdare(A,C,Q,R,S)

% Compute innovations form parameters for a state space model in general form by
% solution of a discrete algebraic Riccati equation (DARE)
%
...
% P         - stabilising solution of the DARE
%
...
% necessary.
%
% See also Matlab 'dare' and 'idare' (this routine is hacked from Matlab 'dare')
...
if ~qzflag || any(~sis(1:r,:)) || any(sis(r+1:rr,:))
    rep = -1;
    return % IMPORTANT: caller must test!!! (error is: ''DARE: eigenvalues on or near unit circle'')
end
